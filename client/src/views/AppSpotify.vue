<template>
    <div>
        ...
        <div v-if="!accessToken" class="btn-container">
            <a v-bind:href="`https://hearthapp.herokuapp.com/api/v1/spotify/login?room=${this.$route.params.roomid}`" class="btn btn-login"><v-icon>$vuetify.icons.spotify</v-icon> Login with Spotify</a>
        </div>
    </div>
</template>
